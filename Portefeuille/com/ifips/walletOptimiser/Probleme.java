...
public class Probleme {
	...
	public Probleme(Fonction fct){
		fonctionObjective = fct;
		mesContraintes = new Vector<Contrainte>();		
	}
	
	public void ajouterContrainte(Contrainte c) throws Exception{
		for (Iterator it = c.partieGauche.getMesVariables().getMesNomVariables().iterator(); it.hasNext();) {
			String el = (String) it.next();
			if(!fonctionObjective.getMesVariables().contains(el)){
	...
	public void afficherProbleme(){
		...
		int nbtest = 0;
		for (Iterator iter = fonctionObjective.getMesVariables().getMesNomVariables().iterator(); iter.hasNext();) {
			String element = (String) iter.next();
			valeur.add(0.0);
		}
		try {
			solutionRetour.setSolution(valeur, fonctionObjective.getMesVariables());
			while(!solutionRetour.estAdmissible()){
				nbContrainteRealise = solutionRetour.getNbContrainteRespecter();
				variableAModifier = fonctionObjective.getMesVariables().getMesNomVariables().get(i);
